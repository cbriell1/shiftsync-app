...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  fetchMessages: async (userId: string) => { if (!userId) return; try { const res = await fetch(`/api/messages?userId=${userId}&t=${Date.now()}`); if (!res.ok) { console.error('fetchMessages failed:', res.status); return; } const data = await res.json(); set({ messages: Array.isArray(data) ? data :[] }); } catch (e) { console.error('fetchMessages error:', e); } },
  fetchAnnouncements: async (userId: string) => { if (!userId) return; try { const res = await fetch(`/api/announcements?userId=${userId}&t=${Date.now()}`); if (!res.ok) { console.error('fetchAnnouncements failed:', res.status); return; } const data = await res.json(); set({ announcements: Array.isArray(data) ? data :[] }); } catch (e) { console.error('fetchAnnouncements error:', e); } },
