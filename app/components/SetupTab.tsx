...
export default function SetupTab({ appState }: any) {
  ...
  return (
    <div className="flex flex-col h-full space-y-4 animate-in fade-in duration-300">
      ...
      <div className="bg-white p-2 md:px-6 md:pt-4 rounded-2xl border border-gray-300 shadow-sm flex gap-4 overflow-x-auto shrink-0">
        <button onClick={() => setActiveSubTab('builder')} className={`py-2 px-4 font-black text-sm outline-none transition-colors border-b-4 whitespace-nowrap ${activeSubTab === 'builder' ? 'border-blue-600 text-blue-900' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>Schedule Builder</button>
        <button onClick={() => setActiveSubTab('templates')} className={`py-2 px-4 font-black text-sm outline-none transition-colors border-b-4 whitespace-nowrap ${activeSubTab === 'templates' ? 'border-blue-600 text-blue-900' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>Shift Templates</button>
        <button onClick={() => setActiveSubTab('tasks')} className={`py-2 px-4 font-black text-sm outline-none transition-colors border-b-4 whitespace-nowrap ${activeSubTab === 'tasks' ? 'border-blue-600 text-blue-900' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>Facility Master Tasks</button>
      </div>
      ...
      <div className="flex-1">
        ...
        {/* VIEW 1: BUILDER */}
        {activeSubTab === 'builder' && <ScheduleBuilderTab />}
