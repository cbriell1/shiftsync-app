...
export default function SetupTab({ appState }: { appState: AppState }) {
  ...
  const toggleTplDay = (idx: number) => { 
    if (editingTplId) { setTplDays([idx]); return; }
    setTplDays(prev => prev.includes(idx) ? prev.filter(x => x !== idx) : [...prev, idx]);
  };

  const toggleTplTask = (taskName: string) => { 
    if (tplTasks.includes(taskName)) setTplTasks(tplTasks.filter(t => t !== taskName)); 
    else setTplTasks([...tplTasks, taskName]); 
  };
  ...
  const handleAddMasterTask = async () => {
    ...
    if (res.ok) { setNewTaskStr(''); triggerSync(); } 
    else { const data = await res.json(); alert(data.error || "Failed to add task."); }
  };
  ...
  const handleEditMasterTask = async (id: number, newName: string) => {
    ...
    if (res.ok) { setEditingTaskId(null); triggerSync(); } 
    else { const data = await res.json(); alert(data.error || "Failed to edit task."); }
  };
      ...
      <div className="flex border-b border-gray-200 mb-6">
        <button
          ...
          className={`py-2 px-6 font-black text-sm outline-none transition-colors border-b-2 ${
            activeTab === 'templates' ? 'border-blue-600 text-blue-800' : 'border-transparent text-slate-600 hover:text-slate-900 hover:border-slate-400'
          }`}
        >
          Shift Templates
        </button>
        <button
          ...
          className={`py-2 px-6 font-black text-sm outline-none transition-colors border-b-2 ${
            activeTab === 'tasks' ? 'border-blue-600 text-blue-800' : 'border-transparent text-slate-600 hover:text-slate-900 hover:border-slate-400'
          }`}
        >
                ...
                <div className="max-h-32 overflow-y-auto bg-white p-2 border-2 border-slate-300 rounded-lg shadow-sm space-y-1">
                  {managerLocations?.map((loc: Location) => (
                    <label key={loc.id} className="flex items-center space-x-2 cursor-pointer text-sm hover:bg-slate-50 p-1.5 rounded">
                      <input 
                        type="checkbox" 
                        checked={tplLocs.includes(loc.id)} 
                ...
                <div className="max-h-40 overflow-y-auto bg-white p-2 rounded-lg border-2 border-slate-300 shadow-sm space-y-1">
                  {globalTasks?.map((task: GlobalTask) => (
                    <label key={task.id} className="flex items-center space-x-2 cursor-pointer hover:bg-slate-50 p-1.5 rounded">
                      <input 
                        type="checkbox" 
                        checked={tplTasks.includes(task.name)} 
                    ...
                    {sortedTemplates?.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="p-10 text-center text-slate-600 font-bold italic bg-slate-50/80">
                          {tplViewLocs.length > 0 || tplViewDays.length > 0 ? "No templates match your active filters." : "No templates found. Create one using the form on the left."}
                        </td>
                      </tr>
                    ) : (
                      sortedTemplates?.map((tpl) => (
                        <tr key={tpl.id} className="border-b border-slate-200 hover:bg-slate-100 transition-colors">
                          ...
                          <td className="p-3 font-bold whitespace-nowrap">
                            {!(tpl.startDate || tpl.endDate) ? <span className="text-green-800">Always</span> : <span className="text-slate-800">{formatDateForTable(tpl.startDate) || 'Always'} to {formatDateForTable(tpl.endDate) || 'Always'}</span>}
                          </td>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-300 overflow-hidden flex flex-col h-full">
            <div className="p-5 md:p-6 border-b border-slate-200 bg-slate-50">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                />
                <button 
                  type="button" onClick={handleAddMasterTask} disabled={!newTaskStr.trim()}
                  className="bg-slate-900 text-white font-black px-6 py-2.5 rounded-lg text-sm disabled:opacity-50 hover:bg-black transition-colors shadow-sm whitespace-nowrap"
                >+ Add Task</button>
              </div>
            </div>
                    ...
                    <tr key={task.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-4 py-1">
                        {editingTaskId === task.id ? (
                          <input
                            value={editTaskStr} onChange={(e) => setEditTaskStr(e.target.value)}
                            onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleEditMasterTask(task.id, editTaskStr.trim()); } }}
                            autoFocus className="w-full bg-white border border-blue-400 rounded px-2 py-1 text-xs font-bold text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-100 shadow-inner"
                          />
