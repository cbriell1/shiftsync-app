  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
        ...
        <button onClick={() => setIsCreateOpen(true)} className="flex items-center gap-1.5 bg-purple-700 hover:bg-purple-800 text-white px-4 py-2 rounded-lg font-black transition-colors shadow-sm text-xs">
            ...
            <div key={statusGroup} className="bg-slate-100/80 border border-slate-200 rounded-xl p-2.5 flex flex-col gap-2.5 max-h-[75vh] overflow-y-auto shadow-inner">
              <div className="flex justify-between items-center px-1 pt-1 pb-1.5 border-b border-slate-200 sticky top-0 bg-slate-100/90 backdrop-blur z-10">
                <h3 className="text-xs font-black text-slate-800 tracking-tight flex items-center gap-1.5">
                  <span className={`w-2 h-2 rounded-full ${statusGroup === 'OPEN' ? 'bg-amber-500' : statusGroup === 'IN PROGRESS' ? 'bg-blue-500' : 'bg-green-500'}`} />
                  {statusGroup}
                </h3>
              ...
              {isFeedbacksLoading ? (
                <>
                  {[1,2].map(i => (
                    <div key={i} className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm animate-pulse space-y-2">
                       <div className="flex justify-between"><div className="h-4 w-12 bg-slate-200 rounded"></div><div className="h-3 w-20 bg-slate-200 rounded"></div></div>
                ...
                filteredFeedbacks.map(fb => {
                  ...
                  return (
                    <div key={fb.id} className={`bg-white border rounded-lg p-2.5 shadow-sm hover:shadow transition-all flex flex-col gap-2 group relative ${shouldHighlight ? 'border-purple-400 ring-1 ring-purple-400 bg-purple-50/40' : 'border-slate-200'}`}>
                      {shouldHighlight && <span className="absolute -top-2.5 -right-2 bg-purple-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full shadow-md animate-pulse z-10 uppercase tracking-widest">Updated</span>}
                      <div className="flex justify-between items-start">
                        <span className={`text-[8px] font-black px-1.5 py-0.5 rounded border uppercase tracking-wider ${fb.type === 'BUG' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>{fb.type}</span>
                        <span className="text-[9px] font-bold text-slate-400">{user?.name?.split(' ')[0] || 'Unknown'} • {createdDate}</span>
                      </div>
                      <p className="text-xs font-medium text-slate-800 leading-snug">{fb.description}</p>
                      ...
                      {isEditing ? (
                        <div className="mt-1 pt-2 border-t border-slate-100 flex flex-col gap-1.5">
                          <select value={editStatus} onChange={(e) => setEditStatus(e.target.value as Feedback['status'])} className="w-full border border-slate-300 rounded p-1 text-[10px] font-bold text-slate-900 bg-slate-50 outline-none">
                            <option value="OPEN">Open</option>
                            <option value="IN PROGRESS">In Progress</option>
                            <option value="COMPLETED">Completed</option>
                          </select>
                          <textarea value={editNotes} onChange={(e) => setEditNotes(e.target.value)} placeholder="Dev notes..." className="w-full border border-slate-300 rounded p-1.5 text-[10px] font-medium text-slate-900 bg-slate-50 outline-none resize-none" rows={2} />
                          <div className="flex gap-1">
                            <button onClick={() => handleSave(fb.id)} disabled={isSaving} className="flex-1 bg-slate-800 text-white font-bold py-1 rounded text-[10px] hover:bg-slate-900 transition-colors">{isSaving ? '...' : 'Save'}</button>
                            <button onClick={cancelEditing} className="flex-1 bg-slate-200 text-slate-700 font-bold py-1 rounded text-[10px] hover:bg-slate-300 transition-colors">Cancel</button>
                          </div>
                        </div>
                      ) : (
                        <button onClick={() => startEditing(fb)} className="text-[10px] font-black text-slate-500 hover:text-slate-800 text-left mt-1 w-max transition-colors opacity-0 group-hover:opacity-100">Manage Ticket</button>
                      )}
                    </div>
                  );
                })
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={(e) => { if(e.target === e.currentTarget) setIsCreateOpen(false); }}>
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in duration-200 border border-slate-300">
            <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setIsCreateOpen(false)} className="text-slate-400 hover:text-red-500 text-lg font-black transition-colors leading-none">&times;</button>
            </div>
            <form onSubmit={handleCreateSubmit} className="p-4 space-y-4">
              <div className="flex p-0.5 bg-slate-100 rounded-lg border border-slate-200">
                <button type="button" onClick={() => setNewType('BUG')} className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 text-[10px] font-black rounded-md transition-all ${newType === 'BUG' ? 'bg-white shadow text-red-700 border border-slate-200' : 'text-slate-500 hover:text-slate-800'}`}>🐞 Report Bug</button>
                <button type="button" onClick={() => setNewType('SUGGESTION')} className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 text-[10px] font-black rounded-md transition-all ${newType === 'SUGGESTION' ? 'bg-white shadow text-blue-700 border border-slate-200' : 'text-slate-500 hover:text-slate-800'}`}>💡 Suggestion</button>
              </div>
              <div>
                <textarea required autoFocus minLength={5} value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder={newType === 'BUG' ? "Describe the issue..." : "Describe your idea..."} className="w-full border border-slate-300 rounded-lg p-2.5 text-xs font-medium text-slate-900 focus:border-purple-500 focus:ring-1 outline-none shadow-inner min-h-[80px] resize-none" />
              </div>
              <button type="submit" disabled={isSubmitting || newDesc.length < 5} className="w-full bg-purple-700 text-white py-2 rounded-lg font-black text-xs shadow-sm hover:bg-purple-800 disabled:opacity-50 transition-colors">
