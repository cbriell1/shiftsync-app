...
export default function StadiumAssistant() {
  ...
  const onDragStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    dragOrigin.current = { mouseX: clientX, mouseY: clientY, offsetX: dragOffset.x, offsetY: dragOffset.y };
    setIsDragging(true);
  };
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent | TouchEvent) => {
      ...
      setDragOffset({
        x: dragOrigin.current.offsetX + (clientX - dragOrigin.current.mouseX),
        y: dragOrigin.current.offsetY + (clientY - dragOrigin.current.mouseY),
      });
    };
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 w-80 md:w-[450px] bg-white border-4 border-slate-900 rounded-[32px] shadow-2xl flex flex-col z-[9999] transition-[height] overflow-hidden ${isMinimized ? 'h-16' : 'h-[600px]'}`}
      style={{ transform: `translate(${dragOffset.x}px, ${dragOffset.y}px)` }}
    >
      {/* HEADER — drag handle */}
      <div
        className="bg-slate-900 p-4 flex items-center justify-between text-white border-b-4 border-yellow-400 shrink-0 select-none"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-black text-xs tracking-widest sports-slant leading-none">Stadium</h3>
            <p className="font-black text-yellow-400 text-[10px] tracking-tighter leading-none mt-0.5">Assistant</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleClose} className="p-1 hover:bg-white/10 rounded transition-colors text-red-400">
            <X size={18} />
          </button>
        </div>
      </div>
              ...
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[90%] p-4 rounded-3xl text-[12px] leading-relaxed shadow-md border-2 transition-all ${
                  ...
                  <div className="prose prose-sm max-w-none prose-slate">
                    <ReactMarkdown
                      components={{
                        h3: ({node, ...props}) => <h3 className="text-[13px] font-black sports-slant text-blue-700 mt-3 mb-1" {...props} />,
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white border-2 border-slate-200 p-4 rounded-3xl rounded-tl-none shadow-sm flex gap-1.5 items-center">
                  <span className="text-[10px] font-black tracking-widest text-slate-400 animate-pulse">Analyzing Stadium Data...</span>
          ...
          <form onSubmit={handleSend} className="p-4 border-t-4 border-slate-100 bg-white shrink-0 shadow-[0_-10px_25px_rgba(0,0,0,0.02)]">
            <div className="relative flex items-center">
              <input
                ...
                className="w-full bg-slate-100 border-2 border-slate-200 rounded-2xl px-5 py-4 text-[13px] font-medium outline-none focus:border-blue-600 transition-all pr-14 shadow-inner placeholder:text-slate-400"
              />
              <button
                type="submit"
